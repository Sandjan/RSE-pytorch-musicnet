...
epsilon = 10e-6  # fudge factor for normalization
...
class MusicNet(data.Dataset):
    ...
    raw_folder = "raw"
    train_data, train_labels, train_tree = (
        "train_data",
        "train_labels",
        "train_tree.pckl",
    )
        ...
        for record in os.listdir(self.data_path):
            ...
            if self.mmap:
                ...
                self.records[int(record[:-4])] = (
                    os.path.join(self.data_path, record),
                    os.fstat(f.fileno()).st_size // sz_float,
                )
                f.close()
        return self
    ...
    def access(self, rec_id, s, shift=0, jitter=0):
        ...
        scale = 2.0 ** ((shift + jitter) / 12.0)

        if self.mmap:
            x = np.frombuffer(
                self.records[rec_id][0][
                    s * sz_float : int(s + scale * self.window) * sz_float
                ],
                dtype=np.float32,
            ).copy()
        else:
        ...
        x = np.interp(scale * xp, np.arange(len(x), dtype=np.float32), x).astype(
            np.float32
        )

        x = rfft(x)[0 : (self.window // 2)].view(dtype="float32")

        y = np.zeros((self.window, self.m), dtype=np.float32)
        for i, pos in enumerate(scale * xp):
            for label in self.labels[rec_id][(s + pos) * (44100 / self.sr)]:
                note = label.data[1] + shift
                if 0 <= note < 128:
                    y[i][note] = 1
    ...
    def __getitem__(self, index):
        ...
        s = np.random.randint(
            0,
            self.records[rec_id][1] - (2.0 ** ((shift + jitter) / 12.0)) * self.window,
        )
    ...
    def _check_exists(self):
        return (
            os.path.exists(os.path.join(self.root, self.train_data))
            and os.path.exists(os.path.join(self.root, self.test_data))
            and os.path.exists(
                os.path.join(self.root, self.train_labels, self.train_tree)
            )
            and os.path.exists(
                os.path.join(self.root, self.test_labels, self.test_tree)
            )
    ...
    def download(self):
        ...
        if not all(
            map(
                lambda f: os.path.exists(os.path.join(self.root, f)),
        ...
        with open(
            os.path.join(self.root, self.train_labels, self.train_tree), "wb"
        ) as f:
            pickle.dump(trees, f)
